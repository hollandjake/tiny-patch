...
describe("deepClone - readonly support", () => {
    test("clones a deeply-frozen object without throwing", () => {
        const src = deepFreeze<Json>({ a: 1, b: { c: [1, 2, { d: 3 }] } });
        expect(deepClone(src)).toEqual({ a: 1, b: { c: [1, 2, { d: 3 }] } });
    });

    test("clones a deeply-frozen array without throwing", () => {
        const src = deepFreeze([1, { a: 2 }, [3, 4]]);
        expect(deepClone(src)).toEqual([1, { a: 2 }, [3, 4]]);
    });

    test("the clone of a frozen input is itself mutable", () => {
        const src = deepFreeze({ a: { b: 1 } });
    ...
    test("accepts a readonly-typed value without a cast", () => {
        // Note: the value must be typed as `DeepReadonly<...>` via a literal (contextual typing)
        // rather than produced by a generic `deepFreeze<T>(val: T): DeepReadonly<T>` helper -
        // TypeScript's inference for `MaybeReadonly<T>` parameters does not reliably narrow T
        // back to the mutable shape in that case, so such a helper's output fails to type-check
        // against `MaybeReadonly<Json>`-based signatures even though the runtime shape is fine.
        const src: DeepReadonly<{ a: number; b: string[] }> = deepFreeze({ a: 1, b: ["x"] });
        // this is a compile-time check: deepClone must accept `src` as-is (no cast/error)
        const clone = deepClone(src);
        // NOTE: generic inference for a `DeepReadonly<T>`-typed argument does not narrow T back
        // to `{ a: number; b: string[] }` here - it widens to the `Json` constraint, so `clone`'s
        // type is the broad `DeepMutable<Json | undefined>`, not the specific input shape. Flagged
        // to the user as a follow-up rather than encoded as intended behavior.
        expectTypeOf(clone).not.toBeNever();
        expect(clone).toEqual({ a: 1, b: ["x"] });
    });

    test("still accepts an ordinary mutable value", () => {
        const src = { a: 1, b: ["x"] };
        expectTypeOf(deepClone(src)).toEqualTypeOf<{ a: number; b: string[] }>();
        expect(deepClone(src)).toEqual(src);
    });
});
